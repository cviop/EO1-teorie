...
\begin{equation}
    y(t) = \mathcal{S}\{u(t)\} ~\Rrightarrow~ay(t) = \mathcal{S}\{au(t)\} = a\mathcal{S}
\end{equation}

\textbf{Princip superpozice}: součet odezev na dílčí složky signálu je odezva na celkový signál\\
\begin{equation}
    y(t) = \mathcal{S}\{au_1(t) + bu_2(t)\} = y_1(t) + y_2(t) = a\mathcal{S}\{u_1(t)\} + b\mathcal{S}\{u_2(t)\}
\end{equation}

\textbf{Časová invariance}: je jedno, kdy do signálu pustíme signál
\begin{equation}
    y(t-t_0) = \mathcal{S}\{u(t-t_0)\}
\end{equation}

Z přenosové funkce jde zjistit
\begin{itemize}
    \item Stabilita systému
    \item Koeficienty modulové charakteristky (nuly a póly)
\end{itemize}
...
\textbf{Příklad}\\
\begin{equation}
    \text{H}(s)=\frac{A\cdot s}{s+B} ~~\rightarrow~~\text{pól }= -B
\end{equation}
Pokud je $Re{B}$ kladné $\rightarrow$ pól je záporný $\rightarrow$ systém je stabilní.
...
\begin{table}[H]
    \begin{tabular}{l}
        ...
        $\omega_0 = \frac{1}{\sqrt{LC}}$ - resonanční frekvence\\
        $Q = \omega_0 \frac{L}{R}$ - činitel jakosti\\
...
Pro výstup $u_\text{1}$ ze sumátoru $\bigoplus$ platí
\begin{equation*}
...
\begin{equation*}
    u_o = A(u_i + \beta u_o) = \frac{u_i A}{1-\beta A}.
\end{equation*}
...
\label{chap:komp}
BACHA! tady už neplatí mantra o OZ z kapitoly~\ref{chap:opamp}, že na vstupech si OZ hlídá stejné napětí. Toto není zapojení zesilovače se ZZV a~nechováme se k chudákovi operákovi hezky. \\
...
Pouze pro moment překlopení platí, že na vstupech je stejné napětí, tedy $u_+ = 0$. Překlápecí úrovně jsou poté:
\begin{gather}
    \begin{aligned}
        \frac{U_i^+}{R_1} = \frac{-U_o^-}{R_2}~\rightarrow~\underline{\underline{U_i^+ = -U_o^-\frac{R_1}{R_2}}}\\
        \frac{U_i^-}{R_1} = \frac{-U_o^+}{R_2}~\rightarrow~\underline{\underline{U_i^- = -U_o^+\frac{R_1}{R_2}}}
    \end{aligned}     
\end{gather}
...
\subsection*{Pár důležitých vztahů na začátek}
\begin{align*}
    I_D &= \frac{1}{2} K_p \frac{W}{L}(U_{GS} - U_{TO})^2\\
